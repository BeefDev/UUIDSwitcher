package io.github.beefdev.uuidswitcher.versions.v1_12_2_R0_1;
...
public final class USLoginListener extends LoginListener implements PacketLoginInListener, ITickable {
    ...
    public void disconnect(IChatBaseComponent ichatbasecomponent) {
        try {
            c.info("Disconnecting {}: {}", this.c(), ichatbasecomponent.toPlainText());
            this.networkManager.sendPacket(new PacketLoginOutDisconnect(ichatbasecomponent));
            this.networkManager.close(ichatbasecomponent);
        } catch (Exception var3) {
            c.error("Error whilst disconnecting player", var3);
            ...
            if (this.server.aG() >= 0 && !this.networkManager.isLocal()) {
                this.networkManager.sendPacket(new PacketLoginOutSetCompression(this.server.aG()), new ChannelFutureListener() {
                    public void a(ChannelFuture channelfuture) throws Exception {
                        USLoginListener.this.networkManager.setCompressionLevel(USLoginListener.this.server.aG());
                    }
    ...
    public void a(IChatBaseComponent ichatbasecomponent) {
        c.info("{} lost connection: {}", this.c(), ichatbasecomponent.toPlainText());
    }
        ...
        if (!Arrays.equals(this.e, packetlogininencryptionbegin.b(privatekey))) {
            ...
            (new Thread("User Authenticator #" + b.incrementAndGet()) {
                public void run() {
                    ...
                    try {
                        String s = (new BigInteger(MinecraftEncryption.a("", USLoginListener.this.server.O().getPublic(), USLoginListener.this.loginKey))).toString(16);
                        USLoginListener.this.i = USLoginListener.this.server.az().hasJoinedServer(new GameProfile((UUID)null, gameprofile.getName()), s, this.a());
                ...
                private InetAddress a() {
                    ...
                    return USLoginListener.this.server.ac() && socketaddress instanceof InetSocketAddress ? ((InetSocketAddress)socketaddress).getAddress() : null;
                }
            }).start();
        }
    }
    ...
    public class LoginHandler {
        ...
        public void fireEvents() throws Exception {
            ...
            USLoginListener.c.info("UUID of player {} is {}", USLoginListener.this.i.getName(), USLoginListener.this.i.getId());
            USLoginListener.this.g = USLoginListener.EnumProtocolState.READY_TO_ACCEPT;
        }
    }
}
